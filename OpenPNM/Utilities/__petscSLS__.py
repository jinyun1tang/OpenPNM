...
class petscSparseLinearSolver:
    ...
    def _initialize_petsc_sparse_coef_mat(self):
        ...
        self.petsc_A = PETSc.Mat().createAIJ(size=self.A.shape, \
        csr=(self.A.indptr[self.Istart:self.Iend+1] - \
        self.A.indptr[self.Istart], \
        self.A.indices[self.A.indptr[self.Istart]:self.A.indptr[self.Iend]], \
        self.A.data[self.A.indptr[self.Istart]:self.A.indptr[self.Iend]]))
    ...
    def _create_petsc_sparse_linear_solver(self):
        ...
        petsc_iterative_solvers= \
        ['richardson','chebyshev','cg','groppcg','pipecg', \
        'pipecgrr','cgne','nash','stcg','gltr','fcg', \
        'pipefcg','gmres','pipefgmres','fgmres','lgmres', \
        'dgmres','pgmres','tcqmr','bcgs','ibcgs','fbcgs', \
        'fbcgsr','bcgsl','pipebcgs','cgs','tfqmr','cr', \
        'pipecr','lsqr','preonly','qcg','bicg','minres','symmlq', \
        'lcd','python','gcr','pipegcr','tsirm','cgls','fetidp']

        # http://www.mcs.anl.gov/petsc/petsc-current/docs/manualpages/PC/PCType.html#PCType
        petsc_preconditioner_methods= \
        ['none','jacobi','sor','lu','shell','bjacobi','mg', \
        'eisenstat','ilu','icc','asm','gasm','ksp','composite', \
        'redundant','spai','nn','cholesky','pbjacobi','mat', \
        'hypre','parms','fieldsplit','tfs','ml','galerkin', \
        'exotic','cp','bfbt','lsc','python','pfmg','syspfmg', \
        'redistribute','svd','gamg','sacusp','sacusppoly', \
        'bicgstabcusp','ainvcusp','chowiluviennacl', \
        'rowscalingviennacl','saviennacl', \
        'bddc','kaczmarz','telescope']

        petsc_lu_direct_solvers= \
        ['mumps','superlu_dist','umfpack','klu']

        petsc_cholesky_direct_solvers= \
        ['mumps','cholmod']
    ...
    def petsc_solve_sparse_linear_sys(self, solver_type='cg', \
        ...
        >>> ls = psls(alg.A, alg.b)
        >>> sol=psls.petsc_solve_sparse_linear_sys(ls)
        ...
        # Destroy petsc solver, coefficients matrix,
        # and rhs and solution vectors
